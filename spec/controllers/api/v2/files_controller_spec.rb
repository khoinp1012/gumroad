...
describe Api::V2::FilesController do
  ...
  describe "POST 'presign'" do
    before do
      @action = :presign
      @params = { filename: "course.pdf", file_size: 1024 * 1024 } # 1 MB
    ...
    context "with edit_products scope" do
      ...
      it "returns 400 when filename is missing" do
        post @action, params: @params.except(:filename).merge(access_token: @token.token)
        expect(response.status).to eq(400)
        expect(response.parsed_body["error"]).to be_present
      end

      it "returns 400 when file_size is missing" do
        post @action, params: @params.except(:file_size).merge(access_token: @token.token)
        expect(response.status).to eq(400)
        expect(response.parsed_body["error"]).to be_present
      end
  ...
  describe "POST 'complete'" do
    before do
      @key = "attachments/#{@user.external_id}/abc123hex/original/course.pdf"
      @action = :complete
      @params = {
